describe 'Header', ->
	...
	describe 'Menu', ->
		...
		xit 'should have one menu item, named "Account Usage Reports"', =>
			expect(@items.length).toBe 1
			expect(@items[0].$('a').getText()).toBe 'Account Usage Reports'

		xit '"Account Usage Reports" menu item should lead to /usage-dashboard', =>
			@items[0].click().then ->
				browser.getCurrentUrl().then (url) ->
					expect(url.indexOf '/usage-dashboard').toBeGreaterThan length()

		xit '"Account Usage Reports" menu item has to be highlighted', =>
