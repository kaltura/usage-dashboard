	...
	module.service 'utils', (columns) ->
		_.extend @,
			...
			arrToObjByField: (arr, field) ->
				obj = {}
				obj[i[field]] = i for i in arr
				obj

			arrToObjOfArrByField: (arr, field) ->
				obj = {}
				for i in arr
					obj[i[field]] = [] unless obj[i[field]]?
					obj[i[field]].push i
				obj

			arrToObj: (arr) ->
				obj = {}
				obj[i] = i for i in arr
				obj

			objToArr: (obj) ->
				return obj if _.isArray obj
				arr = []
				arr.push v for k, v of obj
				arr
			...
			csv:
				extractPayload: (dates, name) =>
					payload = @reports.extractPayload dates
					_.extend payload,
